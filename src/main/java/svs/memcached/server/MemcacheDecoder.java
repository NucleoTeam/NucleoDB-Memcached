...
public class MemcacheDecoder extends ReplayingDecoder<Void> {
    ...
    private void decodeDeleteCommand(ByteBuf in, List<Object> out) {
        logger.debug("Decoding Delete command");
        int length = in.bytesBefore((byte) '\r');
        String key = readString(in, length);
        if(key.endsWith(" noreply")){
            key = key.substring(0, length-" noreply".length());
        }
        in.skipBytes(2); // \r\n
        out.add(MemcacheInboundCommand.newDeleteCommand(key));
    }
    ...
    private void decodeSetCommand(ByteBuf in, List<Object> out) {
        ...
        in.readBytes(data);
        in.skipBytes(2); // \r\n
        out.add(MemcacheInboundCommand.newSetCommand(key, new String(data, StandardCharsets.UTF_8), flags, targetTime));
    }
